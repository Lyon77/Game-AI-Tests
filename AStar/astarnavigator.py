		...
		if item[2] not in closed:
			...
			for edge in network:
				if item[2] == edge[0]:
					copy = item[1][:]
					copy.append(edge[1])
					open.append((item[0] + euDis(edge[1], goal), copy, edge[1]))
				elif item[2] == edge[1]:
					copy = item[1][:]
					copy.append(edge[0])
					open.append((item[0] + euDis(edge[0], goal), copy, edge[0]))
...
def myCheckpoint(nav):
	### YOUR CODE GOES BELOW HERE ###s
	for p in range(len(nav.path) - 1):
		if rayTraceWorld(nav.path[p], nav.path[p+1], nav.world.getGates()) != None:
			nav.setPath(None)
			break
